 ...
 * `true` — `ctx.setValue(SUPPRESS_FETCH_SPAN_KEY, true)` — so a wrapped call
 * that internally uses fetch yields exactly one span. Same mechanism as
 * OpenLLMetry's `suppressTracing` context usage.
 */
