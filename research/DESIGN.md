...
| Q3 | **DEFERRED (M7).** v0 ships "missing" — both tiers verifiably emit usage-less OpenAI streaming spans (M6 unit tests); decide before GA. — OpenAI streaming usage at fetch tier: accept missing usage, inject `stream_options.include_usage` (request mutation — behavior risk), or tiktoken-estimate like OpenLLMetry (`instrumentation-openai/src/instrumentation.ts:698-719`, heavyweight dep)? v0 ships "missing"; decide before GA | openllmetry-js openai instrumentation L592-789; OpenAI API docs |
| Q4 | **DEFERRED (M7).** Mock traffic (Tests B/C) cannot validate clustering quality; needs real traffic. — `agentgraph.agent.fingerprint` definition (⚠️ invention): is hash(provider, model, tool names, system-prompt prefix) stable enough across prompt-template interpolation? Needs real traffic to validate clustering quality | Our own Test B traces; no reference repo answers this |
| Q5 | **DEFERRED (M7).** Test C passed with zero content-level mapping — only needed if/when the graph layer reads message content. — Content-level equivalence mapping `ai.prompt.messages` (AI SDK v6) → `gen_ai.input.messages` JSON shape — needed only if the graph layer reads message content, not for Test C's query contract | ai@6.0.116 `stringifyForTelemetry` (raw/1C §1C.2) vs openllmetry `packages/instrumentation-utils/src/message-formatters.ts:95-142` |
| Q6 | **DEFERRED (M7) — product decision.** Should `withAgent` also emit Traceloop-compatible `traceloop.workflow.name`/`gen_ai.agent.name` for ecosystem interop (Traceloop UI, etc.)? Zero cost via the same processor | openllmetry `span-processor.ts:196-198`; decision is product, not technical |
| Q7 | **PARTIALLY ANSWERED (M7).** The §2.3 contract held verbatim on real ai@6.0.203 spans (Test C green); caution recorded: v6 parents `ai.toolCall` under the ROOT `ai.streamText` (FINDINGS §8's under-doStream placement is v7 behavior) — consumers must not depend on tool-span parenting. Full v7 run still unverified. — AI SDK v7 forward-compat: when ai-chatbot upgrades, native spans become `invoke_agent`/`chat`/`execute_tool` via `@ai-sdk/otel` with telemetry **opt-out** — does the Test C query contract (§2.3) hold unmodified? FINDINGS §1.4 says yes for the v7 `OpenTelemetry` integration (it emits `gen_ai.provider.name`/`gen_ai.request.model`/`gen_ai.usage.*`), but unverified against a running v7 app | ai repo `packages/otel/src/open-telemetry.ts:252-296,432-496` |
...
> **M7 status: executed as Test A-lite** — the documented recipe needs Vercel AI Gateway credentials (unavailable at M7). `scripts/test-c/run.ts` runs the same one-tool weather turn through AI SDK v6 `streamText` directly (`test-apps/ai-sdk-native/`, native telemetry, deterministic Anthropic mock). Span tree, parenting, timestamps, and the fixture export below all pass, with one empirical correction: **ai@6.0.203 parents `ai.toolCall` under the root `ai.streamText`**, not under its step's `doStream` (that placement is v7 — FINDINGS §8's citation). Re-export the fixture from a real ai-chatbot run when gateway credentials are available.
...
> **M7 status: GREEN.** The contract ships as `@agentgraph/core` `query-contract.ts` (`isLLMCall`/`provider`/`usage`, unit-tested) and `scripts/test-c/run.ts` (`npm run test:ac`, in CI) executes it against both Jaeger result sets via one code path: A selects exactly the 2 `doStream` spans of 4 (root and `ai.toolCall` rejected), B selects both `chat {model}` spans; provider resolves via the single documented coalesce (`anthropic.messages` vs `anthropic`); usage identical (17/42) everywhere.
